...
/* ===== BACKGROUND CANVAS ===== */
  ...
  /* ===== TICKS — the owner ticks a vaccination / deworming row once it is done.
     ...
     A tick is saved per item-id, so it survives a reload, under the same key
     home.html reads — that lets home.html drop the item from its bell too. ===== */
  ...
  function saveTick(itemId, done) {
    try {
      const all = loadTicks();
      if (done) all[itemId] = { cls: 'done' };
      else delete all[itemId];
      localStorage.setItem(STATUS_STORE_KEY, JSON.stringify(all));
    } catch (e) { /* storage unavailable — the tick won't persist across reloads */ }
  }

  // Shows / hides the tick on one row (and marks the row so the reminder bell skips it)
  function setRowDone(row, done) {
    const box = row.querySelector('input[type="checkbox"]');
    if (box) box.checked = done;
    row.classList.toggle('is-done', done);
    if (done) row.setAttribute('data-done', 'true');
    else row.removeAttribute('data-done');
  }

  // Runs once on page load, before the reminder bell is counted, so ticks
  // saved on a previous visit are back in place straight away.
  function applyTicks() {
    const ticks = loadTicks();
    document.querySelectorAll('tr[data-item-id]').forEach(row => {
      const saved = ticks[row.dataset.itemId];
      setRowDone(row, !!saved && saved.cls === 'done');
    });
  }

  // Called by the tick box on each row
  function toggleDone(box) {
    const row = box.closest('tr');
    if (!row) return;
    setRowDone(row, box.checked);
    if (row.dataset.itemId) saveTick(row.dataset.itemId, box.checked);
    renderReminderBell();
    // lets other scripts (e.g. the Firestore sync) react to a tick
    document.dispatchEvent(new CustomEvent('pawhouse:tick-changed', {
      detail: { petId: PET_ID, itemId: row.dataset.itemId, done: box.checked }
    }));
  }
  ...
  // Collects every vaccination/deworming row the owner has NOT ticked yet
  function collectReminderItems() {
    const rows = document.querySelectorAll('tr[data-remind-type][data-due]');
    const today = todayISO();
    const items = [];
    rows.forEach(row => {
      if (row.dataset.done === 'true') return; // ticked by the owner — nothing to remind
  ...
  // NOTE: home.html reads the same STATUS_STORE_KEY (pawhouse_status_<petId>)
  // written by saveTick() above to know which items are ticked —
  // no separate summary key needed here.
  ...
  function renderReminderBell() {
    ...
    if (items.length > 0) {
      ...
      bell.classList.toggle('has-due', items.some(i => i.isDueToday || i.isOverdue));
    } else {
    ...
    list.innerHTML = items.map(item => `
      <div class="reminder-item">
        ...
        <div class="reminder-item-info">
          ...
          <div class="reminder-item-date">Due ${formatReminderDate(item.due)}</div>
        </div>
      </div>
    `).join('');
  }
  ...
  /* ===== DOCUMENT LOCKER — owner-uploaded pet documents (vaccine certs,
     medical records, adoption papers, etc.), stored as data URLs in
     localStorage under key `pawhouse_docs_<PET_ID>`. No upload cooldown
     (unlike the gallery) since these are reference documents, not a
     rate-limited memory feed — but capped in size so localStorage
     doesn't fill up. ===== */
  const LOCKER_MAX_BYTES = 5 * 1024 * 1024; // 5MB per file
  const LS_DOCS_KEY = `pawhouse_docs_${PET_ID}`;

  function getStoredDocs() {
    try {
      return JSON.parse(localStorage.getItem(LS_DOCS_KEY) || '[]');
    } catch (e) {
      return [];
    }
  }
  ...
  function formatLockerDate(ts) {
    const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    const d = new Date(ts);
    return `${String(d.getDate()).padStart(2,'0')} ${months[d.getMonth()]} ${d.getFullYear()}`;
  }

  function formatLockerSize(bytes) {
    if (bytes >= 1024 * 1024) return `${(bytes / (1024 * 1024)).toFixed(1)} MB`;
    return `${Math.max(1, Math.round(bytes / 1024))} KB`;
  }

  function lockerIconFor(type) {
  ...
  function renderLockerList() {
    ...
    const empty = document.getElementById('lockerEmpty');
    ...
    docs.forEach(doc => {
      ...
      row.className = 'locker-row';
      row.innerHTML = `
        <div class="locker-icon">${lockerIconFor(doc.type)}</div>
        <div class="locker-info">
          <div class="locker-name">${doc.name}</div>
          <div class="locker-meta">${formatLockerDate(doc.date)} · ${formatLockerSize(doc.size)}</div>
        </div>
        <a class="locker-view" href="${doc.dataUrl}" target="_blank" rel="noopener" download="${doc.name}">View</a>
      `;
      list.appendChild(row);
    });
  }

  function handleLockerTileClick() {
    document.getElementById('lockerFileInput').click();
  }

  function handleLockerUpload(event) {
    const file = event.target.files && event.target.files[0];
    event.target.value = ''; // allow re-selecting the same file later
    if (!file) return;
    ...
    if (!okType) {
      showGalleryToast('Please choose a PDF or image file.');
      return;
    }
    if (file.size > LOCKER_MAX_BYTES) {
      showGalleryToast('That file is too large — please keep it under 5MB.');
      return;
    }
    ...
    reader.onload = (e) => {
      const docs = getStoredDocs();
      docs.push({ name: file.name, type: file.type, size: file.size, dataUrl: e.target.result, date: Date.now() });
      try {
        saveStoredDocs(docs);
      } catch (err) {
        showGalleryToast('Storage is full — please remove old documents to add new ones.');
        return;
      }
      renderLockerList();
      showGalleryToast('Document added to the locker 🔒');
    };
    reader.readAsDataURL(file);
  }
